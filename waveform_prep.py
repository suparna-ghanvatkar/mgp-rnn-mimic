...
def dataset_prep():
    ...
    for line in fwaves:
        ...
        try:
            ...
            continue
    sigfile = open('various_signals.pickle','w')
    ...
    for sub in waves.keys():
        ...
        stays = pd.read_csv('~/mgp-rnn-mimic/data/root/'+str(sub)+'/stays.csv', parse_dates=True)
        ...
        for i in range(stays.shape[0]):
            ...
            total_rows = int(ceil(total_time.total_seconds()))
        ...
        stay_pd.to_csv(os.path.join('~/mgp-rnn-mimic/data/waveforms/',str(sub)+"_stay"+str(i)+'.wav'), index=False)
